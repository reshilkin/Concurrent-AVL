...
public class LockBasedFriendlyTreeMap<K, V> extends AbstractMap<K, V> implements
	...
	static final double SPLAY_PROB = 1.0 / 80;
	...
	private void splay(Node<K, V> n, long depth) {
		...
		while (parent != root) {
			Node<K, V> gParent = lockParent(parent);
			if (!parent.removed && parent.value == DELETED && (parent.left == null || parent.right == null)) {
				if (splayRemoveNode(parent)) {
					gParent.lock.unlock();
					break;
				}
			}
			...
			Node<K,V> ggParent = lockParent(gParent);
			if (!gParent.removed && gParent.value == DELETED && (gParent.left == null || gParent.right == null)) {
				if (splayRemoveNode(gParent)) {
					gParent.lock.unlock();
